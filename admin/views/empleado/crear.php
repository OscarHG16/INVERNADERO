...
<form method="post" action="empleado.php?accion=<?php if($accion == "crear"):echo('nuevo');else:
    echo('modificar&id='.$id);endif; ?>" enctype="multipart/form-data">
    <div class="mb-3">
    ...
    <div class="mb-3">
        ...
        <input class="form-control" type="text" name="data[primer_apellido]" placeholder="Escribe tu primer apelllido" 
        id="primer_empleado" value="<?php
        if(isset($empleados['primer_apellido'])):echo($empleados['primer_apellido']);endif; ?>" /> 
    </div>

    <div class="mb-3">
        <label for="segundo_apellido" class="form-label">Segundo apellido</label>
        <input class="form-control" type="text" name="data[segundo_apellido]" placeholder="Escribe tu segundo apellido" 
        id="segundo_empleado" value="<?php
        if(isset($empleados['segundo_apellido'])):echo($empleados['segundo_apellido']);endif; ?>"/>
    </div>

    <div class="mb-3">
        <label class="form-label" for="rfc">RFC</label>
        <input class="form-control" type="text" name="data[rfc]" placeholder="Escribe tu RFC" 
        id="rfc" value="<?php
        if(isset($empleados['rfc'])):echo($empleados['rfc']);endif; ?>"/>
    </div>

    <div class="mb-3">
        <label class="form-label" for="rfc">Fotografia</label>
        <input class="form-control" type="file" name="fotografia" placeholder="Coloca tu fotografia" 
        id="fotografia" value="<?php
        if(isset($empleados['fotografia'])):echo($empleados['fotografia']);endif; ?>"/>
    </div>
